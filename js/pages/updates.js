/* ALLSTAR GALAXY — UPDATES PAGE V4.10
...
function card(c){const has=String(c.graphic||'').trim();return `<section class="updates-module" id="${esc(c.id)}" data-update-id="${esc(c.id)}">
 <button class="updates-live-card" type="button" ${has?'':'aria-disabled="true"'} data-open="${esc(c.id)}">
...
function openAt(i){const c=cards[i];if(!c?.graphic)return;active=i;const box=document.querySelector('#updates-lightbox');box.querySelector('#updates-lightbox-title').textContent=c.title;box.querySelector('.updates-lightbox-time').textContent=`UPDATED • ${stamp(c.updated)}`;box.querySelector('.updates-full-image').innerHTML=`<img src="${esc(c.graphic)}" alt="${esc(c.alt||c.title)}">`;box.hidden=false;document.body.classList.add('updates-modal-open');box.querySelector('.updates-close').focus()}
function close(){document.querySelector('#updates-lightbox').hidden=true;document.body.classList.remove('updates-modal-open')}
function bind(){document.querySelectorAll('[data-open]').forEach(b=>b.addEventListener('click',()=>{const i=cards.findIndex(c=>c.id===b.dataset.open);if(i>=0&&cards[i].graphic)openAt(i)}));const box=document.querySelector('#updates-lightbox');box.querySelectorAll('[data-close]').forEach(x=>x.addEventListener('click',close));box.querySelector('.updates-prev').onclick=()=>{let i=active;do{i=(i-1+cards.length)%cards.length}while(!cards[i].graphic&&i!==active);openAt(i)};box.querySelector('.updates-next').onclick=()=>{let i=active;do{i=(i+1)%cards.length}while(!cards[i].graphic&&i!==active);openAt(i)};document.addEventListener('keydown',e=>{if(box.hidden)return;if(e.key==='Escape')close();if(e.key==='ArrowLeft')box.querySelector('.updates-prev').click();if(e.key==='ArrowRight')box.querySelector('.updates-next').click()});}
