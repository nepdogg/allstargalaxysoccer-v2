/* ============================================================================
   ALLSTAR GALAXY V2.9 — UNIVERSAL SITE FRAME
...
export function mountSiteFrame(site,page){
 ...
 host.innerHTML=`<header class="shared-header"><div class="identity-row"><a class="brand-home brand-home-left" href="index.html"><img class="brand-logo" src="${site.logo}" alt="Allstar Galaxy"></a><img class="nav-title" src="${page.navTitle}" alt="The Official Home of the Allstar Galaxy"><a class="brand-home brand-home-right" href="index.html"><img class="brand-logo" src="${site.logo}" alt="Allstar Galaxy"></a><a class="search-control" href="search.html"><span>⌕</span><b>Search</b></a></div><nav class="main-nav" aria-label="Main navigation">${nav(site,page)}</nav><div class="status-module"><span class="status-border status-border-top"></span><span class="energy-rail energy-rail-top"><i></i></span><div class="status-bar"><span class="status-side status-side-left"><a class="status-open" href="#main">← OPEN</a><span class="status-icon">${page.icon||'★'}</span></span><span class="status-text">${page.ticker||''}</span><span class="status-side status-side-right"><span class="status-icon">${page.icon||'★'}</span><a class="status-open" href="#main">OPEN →</a></span></div><span class="energy-rail energy-rail-bottom"><i></i></span><span class="status-border status-border-bottom"></span></div></header>`;
...
export function mountFooter(site){
 ...
 host.innerHTML=`<div class="footer-inner"><section class="footer-brand-block"><div class="footer-brand-line"><img class="footer-xitlali-logo" src="${site.xitlaliLogo||'assets/images/logos/xitlali-media-logo.png'}" alt="Xitlali Media"><div class="footer-brand-copy"><strong>${site.footer.credit}</strong><small>Preserving every game, player, and memory.</small><em>Capturing Moments. Creating Legacies.</em></div></div></section><nav class="footer-links" aria-label="Footer navigation">${links}</nav><div class="footer-social">${socials}</div><div class="footer-contact-row"><a class="footer-contact" href="mailto:${email}">✉ Contact Administrator</a></div><div class="footer-divider"></div><div class="footer-meta"><span class="footer-copyright"><span>© 2026</span><img class="footer-mini-logo" src="${site.logo}" alt=""><span>Allstar Galaxy</span></span><small>Allstar Galaxy V${site.version||'3.0.0'} • Final Frame and Footer Repair • GitHub Pages</small></div></div>`;
}
